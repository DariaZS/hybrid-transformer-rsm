# Dataset Loading: TinyStories + Needle-in-Haystack
=================================
1. TinyStories: Real research benchmark from Microsoft (2023 paper)
   - Simple stories generated by GPT-4 for training small models
   - Each story is 500-2000 tokens (manageable for our 256-token chunks)
   - Uses GPT-2 BPE tokenizer (50K vocabulary, industry standard)
   
2. Needle-in-Haystack: Testing long-range retrieval
   - Insert a fact (the "needle") at a specific position in long text (the "haystack")
   - Test if model can retrieve the needle after processing entire haystack
   - Used by Anthropic, Google, OpenAI to test context windows
